    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault();
            ...
            if (response?.error) {
                ...
                const errorMessage = response.code?.replace(/^Error:\s*/, '') || null;
                ...
                <Card className="w-full max-w-md bg-white shadow-xl rounded-3xl p-8 text-gray-900 border border-gray-300 animate-pop-in">
                    ...
                    <CardContent>
                        <form className="space-y-6" >
                            ...
                            <div>{error}</div>
                            <motion.div>
                                <Button type="submit" onClick={handleSubmit} className="w-full bg-gradient-to-r from-[#FFD700] to-[#FF8C00] hover:from-[#FFC700] hover:to-[#FF7F00] hover:scale-95 text-white font-semibold py-3  rounded-lg drop-shadow-glow">
                                    Đăng nhập
                                </Button>
                            </motion.div>
                        </form>
